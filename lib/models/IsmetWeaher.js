...
class IsmetWeather {
  ...
  _getIsmetForecast() {
    return this.data
      ...
      .filter((el) => {
        if (el.description !== undefined && el.title.includes(forecast)) {
          return el
        }
      })[0]
  }
  ...
  _getIsmetDroughtStatus() {
    return this.data
      ...
      .filter((el) => {
        if (el.description !== undefined && el.title.includes(droughtStatus)) {
          return el
        }
      })[0]
  }
   ...
   */
  _getCityNames(item) {
    if (item.title !== droughtStatus && !item.title.includes(forecast)) {
